...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { data: purchase, error } = await supabase.from("purchases")
    .select("id,organization_id,created_at:purchased_at,amount_paid,contract_snapshot")
    .eq("id", id).eq("user_id", user.id).maybeSingle();
  if (error) return new Response("No se pudo obtener el justificante. Inténtalo de nuevo.", { status: 503 });
  if (!purchase) return new Response("No encontrado", { status: 404 });
